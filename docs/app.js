/**
 * app.js — InkTeX Web App
 *
 * Identical to popup.js (Chrome extension) except chrome.runtime.getURL()
 * calls are replaced with relative paths for standalone web hosting.
 */
...
const CANVAS_W     = 720;
const CANVAS_H     = 320;
const MODEL_PATH   = "../extension/model/model.onnx";
const CLASSES_PATH = "../extension/model/classes.json";
...
async function init() {
  ...
  try {
    ...
    // Resolve the lib/ path relative to this page so ORT can fetch its WASM binaries.
    window.ort.env.wasm.wasmPaths = new URL("../extension/lib/", window.location.href).href;
...
function findBoundingBoxes(imageData, W, H) {
  ...
  return squares.filter((sq) => sq.valid).sort((a, b) => a.x - b.x);
}
